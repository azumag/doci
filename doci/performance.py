...
def opening_retention_signal(
    ...
    *,
    ...
    """冒頭の指定秒数内の維持率低下と最大低下区間を観測する。

    最初と最後の観測点の差（累計低下）に加え、隣接点間で最も大きい低下を
    返す。どちらかが ``threshold`` 以上なら ``actionable`` とするが、これは
    レポート対象を絞るための検知閾値であり、動画の合否や因果を示さない。

    既定ウィンドウはissue #142の30秒。issue #127のShorts冒頭3秒分析も
    同じ保守的な計算を使う。秒位置を捏造しないため動画長が不明、指定
    ウィンドウ内の有効点が2点未満、または設定値が不正な場合は ``None`` を返す。
    指定秒数より短い動画は全長をウィンドウとして扱う。
    """
