...
- 視聴者層の指標は定義の時間窓ごと確認する。月間視聴者数は基準日から過去28日間のローリング値、
  ライト・コアな視聴者は過去1年の視聴月数（コアは6か月以上）に基づく分類であり、公開直後の
  一本の動画の即時的な成果・定着の証拠と断定する記述は、短期の再訪と中長期の定着を分けた
  控えめな言い方へ直す。
    ...
    if original_narration is not None and rewritten_narration is not None:
        ...
        if isinstance(audit, list):
            for issue in audit:
                ...
                if not decision:
                    decision = "correct" if replacement else "remove"
